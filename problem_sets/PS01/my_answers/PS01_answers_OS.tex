...
\begin{document}
	...
	\begin{enumerate}
		...
		\textit{Although the calculated sample mean of 98.44 is less than the population mean IQ of 100, the counselor's interest in finding whether the average student IQ is "higher than" the population mean implies that this will be a one-sided t-test with n-1 = 24 degrees of freedom.}
		
		\textbf{1. ASSUMPTIONS: }
		
		We can safely assume that the data in question is quantitative, as IQ is measured numerically. 
		...
		The estimated standard error is \textit{se = s / $\sqrt{n}$} = 13.09287 / $\sqrt{25}$ = 13.09287 / 5.
		
		\lstinputlisting[language=R, firstline=126, lastline=126]{PS01_answers_OS.R}
		
		This computes to 2.618574.
		
		The value of the test statistic \textit{t} is the difference between the observed sample mean (98.44) and the population mean (100) divided by the standard error (2.618574).
		
		\lstinputlisting[language=R, firstline=132, lastline=133]{PS01_answers_OS.R}
		
		This computes to \textit{t = }-0.5957439.
		
		In other words, the sample mean falls 0.5957439 estimated standard errors below the null hypothesis value of the mean. 
		
		\textbf{4. P-VALUE:}
		
		I use the "pt" command in R to determine the P-value given the calculated value of \textit{t} and the degrees of freedom \textit{n-1}. I do not multiply the value by 2 because this is a one-sided t-test. 
		
		From the \textit{t}-distribution with \textit{df} = 24, the one-tail probability is:
		
		\lstinputlisting[language=R, firstline=145, lastline=146]{PS01_answers_OS.R}
		
		Using the "pt" command in R, this computes to \textit{P} = 0.2784617.
...
\end{verbatim}
	...
	\textit{	***NOTE: For this question, I opted to create scatterplots in R and create regression models to fit the data. I did so because this is the only way I know how to find correlation (R-squared) values in R. I understand that we have not covered regression formally in class yet, but this is the method I recall from my undergraduate coursework and from the ggplot examples we covered in the Code Camp. As stated in my academic integrity cover sheet, no Gen AI was used here or anywhere else in the assignment, though I did rely on Google searches and StackOverflow for assistance with writing functional code.}
