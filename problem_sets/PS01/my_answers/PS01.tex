...
\begin{document}
	...
	\lstinputlisting[language=R, firstline=51, lastline=51]{PS01.R}  
	...
	\begin{enumerate}
		...
		\lstinputlisting[language=R, firstline=55, lastline=56]{PS01.R}
		...
		The sample size \textit{n} = 25; therefore, the degrees of freedom \textit{df} = \textit{n}-1 = 24. The formula for a confidence interval for a mean is \textit{y-bar} ± \textit{t(s / sqrt(n))}, where \textit{y-bar} is the sample mean, \textit{t} is the critical value from the  \textit{t}-distribution, \textit{s} is the sample standard deviation, and \textit{n} is the sample size. The value \textit{s/sqrt(n)} is the standard error.
		
		As we know \textit{y-bar} (98.44 mean IQ, calculated previously), \textit{s} (13.09287, calculated previously), and \textit{n} (given), the only remaining unknown value is the critical value from the \textit{t}-distribution. 
		...
		Therefore, \textit{y-bar ± t(s / sqrt(n))} = 98.44 ± 1.711(13.09287/\textit{sqrt}(25))
		
		I calculate the lower and upper values of the 90 percent confidence interval and assign them to objects: 
		\lstinputlisting[language=R, firstline=82, lastline=83]{PS01.R}
		
		I return the the lower and upper values of the interval:
		\lstinputlisting[language=R, firstline=86, lastline=87]{PS01.R}
		
		This returns 93.95962 and 102.9204, respectively.
		
		\textbf{The 90 percent confidence interval for the mean IQ score is (93.95962, 102.9204).}
		
		\item Next, the school counselor was curious  whether  the average student IQ in her school is higher than the average IQ score (100) among all the schools in the country.\\ 
		
		\noindent Using the same sample, conduct the appropriate hypothesis test with $\alpha=0.05$.
		
		Although the counselor is wondering whether the mean IQ of students at the school is "higher than" (rather than "different than") the nationwide average IQ of 100, the sample mean of 98.44 is lower than 100. This suggests that, in spite of the counselor's framing, a two-sided significance test, rather than a one- sided test, is appropriate.
		
		\textbf{1. ASSUMPTIONS: }
		
		We can reasonably assume the sample was randomly selected, as the assignment states the counselor "took a random sample." 
		
		We can also reasonably assume the quantitative variable in question (IQ) has a normal population distribution, as "Most people score near the average range, which is designed to fit into a normal distribution curve" (Kendra Cherry, MSEd).
		...
		\textit{Ha: meanIQ} is not equal to 100
		...
		\lstinputlisting[language=R, firstline=126, lastline=126]{PS01.R}
		...
		\lstinputlisting[language=R, firstline=132, lastline=133]{PS01.R}
		...
		The P-value is the two-tail probability,  presuming \textit{Ho} is true, that \textit{t} would exceed 0.5957439 in absolute value.  From the \textit{t}-distribution with \textit{df} = 24, this two-tail probability is:
		
		\lstinputlisting[language=R, firstline=145, lastline=146]{PS01.R}
		
		Using the "pt" command in R (found online through a Google search, no AI  used), this computes to \textit{P} = 0.5569233.
		
		With our \textit{alpha}-level of 0.05, \textit{P} is greater than \textit{alpha}. 
	...
	\lstinputlisting[language=R, firstline=160, lastline=161]{PS01.R}
	
	I generate some preliminary exploratory statistics using the following commands:
	\lstinputlisting[language=R, firstline=165, lastline=168]{PS01.R}
	...
	The "summary," "head," and "glimpse" commands produce the following output:
	
	\textbf{\textit{!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FiNiSh tHiS sEcTiOn !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!}}
	\textbf{\textit{!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FINISH THIS SECTION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!}}
	\textbf{\textit{!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FiNiSh tHiS sEcTiOn !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!}}
	\textbf{\textit{!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FINISH THIS SECTION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!}}
	\textbf{\textit{!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FiNiSh tHiS sEcTiOn !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!}}
	\textbf{\textit{!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FINISH THIS SECTION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!}}
	\textbf{\textit{!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FiNiSh tHiS sEcTiOn !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!}} \textbf{\textit{!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FINISH THIS SECTION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!}}
	\textbf{\textit{!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FiNiSh tHiS sEcTiOn !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!}} \textbf{\textit{!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FINISH THIS SECTION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!}}
	...
	\begin{itemize}
		...
		\lstinputlisting[language=R, firstline=174, lastline=176]{PS01.R}
		
		I generate the below scatterplot using the following code:
		\lstinputlisting[language=R, firstline=182, lastline=191]{PS01.R}
		...
		I create a simple linear regression model and calculate the slope, intercept, and correlation (R-squared):
		\lstinputlisting[language=R, firstline=197, lastline=199]{PS01.R}
		
		I generate the below scatterplot using the following code:
		\lstinputlisting[language=R, firstline=205, lastline=216]{PS01.R}
		...
		\lstinputlisting[language=R, firstline=261, lastline=261]{PS01.R}
		...
		\lstinputlisting[language=R, firstline=268, lastline=269]{PS01.R}
		
		I create the below boxplots using ggplot:
		\lstinputlisting[language=R, firstline=271, lastline=276]{PS01.R}
		...
		\lstinputlisting[language=R, firstline=279, lastline=283]{PS01.R}
		...
		\lstinputlisting[language=R, firstline=292, lastline=295]{PS01.R}
		
		I then create the below scatterplot of Y on X1 using this code:
		\lstinputlisting[language=R, firstline=300, lastline=310]{PS01.R}
		\includegraphics[scale=0.7]{housingByIncomePlot}\\
		...
		\lstinputlisting[language=R, firstline=322, lastline=332]{PS01.R}
